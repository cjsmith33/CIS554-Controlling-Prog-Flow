...
def iseurofloat(s):
    ...
    try:
        x = (s[:introcs.find_str(s,',')])
        y = (s[introcs.find_str(s,',') + 1: len(s)])
        x = int(s[:introcs.find_str(s,',')])
        y = int(s[introcs.find_str(s,',') + 1: len(s)])
        result = True
    except:
        result = False

    result = result and introcs.find_str(s,',') != -1
    return result
